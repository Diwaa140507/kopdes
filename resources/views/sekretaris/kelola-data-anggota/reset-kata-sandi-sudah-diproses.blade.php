<x-sekretaris-layout activeMenu="kelola-data-anggota" headerTitle="Data Anggota — Sekretaris">
    ...
    <table style="width:100%; border-collapse:collapse; font-size:14px;">
        ...
        <tbody>
            @forelse ($riwayat as $item)
                @php $isSelected = $selected && $selected->id_anggota === $item->id_anggota; @endphp
                <tr style="border-bottom:1px solid #E5E7EB; {{ $isSelected ? 'background:#FDEEEE;' : '' }}">
                    ...
                    <td style="padding:10px 12px;">
                        <a href="{{ route('sekretaris.kelola-data-anggota.reset-kata-sandi.sudah-diproses', ['detail' => $item->id_anggota]) }}"
                           style="text-decoration:none;">
                            <span style="background:#EAF7EC; color:#1E7A34; padding:4px 10px; border-radius:4px; font-size:12px; cursor:pointer;">Selesai</span>
                        </a>
                    </td>
                </tr>
            @empty
    ...
    @if ($selected)
        <p style="font-size:13px; color:#6B7280; margin:20px 0 8px;">Detail riwayat — {{ $selected->id_anggota }}</p>
        ...
        <div style="background:#FDEEEE; border:1px solid #F3B4B4; border-radius:6px; padding:20px; max-width:600px;">
            ...
            <div style="margin-top:12px; padding:12px 16px; background:#FCE9C7; border-radius:4px;">
                <p style="margin:0 0 4px; color:#8A5A00; font-size:12px; font-weight:bold;">PASSWORD SEMENTARA</p>
                <p style="margin:0; color:#8A5A00; font-size:16px; font-family:monospace;">{{ $selected->password_sementara_plain ?? '-' }}</p>
            </div>

            <p style="margin:12px 0 0; font-size:12px; color:#6B7280;">Sampaikan password ini secara manual/luring ke anggota. Anggota akan wajib ganti password saat login pertama kali.</p>
        </div>
    @endif

    <p style="font-size:13px; color:#6B7280; margin-top:24px;">
        Klik status "Selesai" pada baris untuk melihat detail riwayat, termasuk password sementara yang diberikan.
    </p>

<x-sekretaris-layout title="Verifikasi Pendaftaran — Sekretaris">

    <h1 style="font-size:22px; font-weight:700; color:#241412; margin:0 0 16px 0;">Verifikasi Pendaftaran Anggota</h1>

    @if (session('success'))
        <div style="background:#EAF7EC; border:1px solid #1E7A34; color:#1E7A34; padding:12px 16px; border-radius:6px; margin-bottom:16px; font-size:14px;">
            {{ session('success') }}
        </div>
    @endif
    ...
    @if ($selected)
        <div style="background:#FDEEEE; border:1px solid #F3B4B4; border-radius:8px; padding:20px; max-width:600px;">
            ...
            <p style="margin:4px 0; font-size:14px;"><strong>No. HP :</strong> {{ $selected->no_hp ?? '-' }}</p>
            ...
            <form method="POST" action="{{ route('sekretaris.verifikasi.tolak', $selected->id_anggota) }}">
                @csrf
                <label style="display:block; font-size:13px; color:#241412; margin-bottom:6px;">Catatan Penolakan (opsional):</label>
                <input type="text" name="catatan_penolakan" placeholder="Isi jika pendaftaran ditolak..."

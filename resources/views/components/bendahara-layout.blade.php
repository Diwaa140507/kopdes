@props(['activeMenu' => 'simpanan', 'headerTitle' => 'Konfirmasi Setoran — Bendahara'])
...
<body style="margin:0; font-family: Arial, sans-serif; background:#F3F4F6;">
    ...
    <div style="display:flex; min-height:calc(100vh - 60px);">
        ...
        <div style="width:220px; background:#ffffff; border-right:1px solid #E5E7EB; padding:20px 0;">
            @php
                $menuItems = [
                    'simpanan' => ['label' => 'Simpanan', 'route' => 'bendahara.simpanan.setoran'],
                    'pinjaman' => ['label' => 'Pinjaman', 'route' => 'bendahara.pinjaman.index'],
                    'cicilan' => ['label' => 'Cicilan', 'route' => 'bendahara.cicilan.index'],
                ];
            @endphp
            ...
            @if ($pengurus->jabatan === 'Ketua Koperasi')
                <div style="margin:16px 12px 0; padding-top:16px; border-top:1px solid #E5E7EB;">
                    <a href="{{ route('ketua.laporan.pilih') }}"
                       style="display:block; padding:12px 20px; border-radius:4px; text-decoration:none; color:#B91C1C; font-weight:bold; border:1px dashed #B91C1C;">
                        ← Kembali ke Menu Ketua
                    </a>
                </div>
            @endif
        </div>
